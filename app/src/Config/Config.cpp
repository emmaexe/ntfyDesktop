...
void Config::write() {
    ...
    if (!std::filesystem::exists(Config::getConfigPath()) || !std::filesystem::is_directory(Config::getConfigPath())) {
...
const std::string Config::getConfigPath() {
    return QStandardPaths::writableLocation(QStandardPaths::ConfigLocation).toStdString() + std::string("/ntfyDesktop/");
}

const std::string Config::getConfigFile() {
    return QStandardPaths::writableLocation(QStandardPaths::ConfigLocation).toStdString() + std::string("/ntfyDesktop/config.json");
}
