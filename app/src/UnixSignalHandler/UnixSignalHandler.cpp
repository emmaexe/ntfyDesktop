...
void UnixSignalHandler::hupSignalHandler(int) {
    char a = 1;
    (void)write(UnixSignalHandler::sighupFileDescriptor[0], &a, sizeof(a));
}

void UnixSignalHandler::intSignalHandler(int) {
    char a = 1;
    (void)write(UnixSignalHandler::sigintFileDescriptor[0], &a, sizeof(a));
}

void UnixSignalHandler::termSignalHandler(int) {
    char a = 1;
    (void)write(UnixSignalHandler::sigtermFileDescriptor[0], &a, sizeof(a));
}

void UnixSignalHandler::handleSigHup() {
    this->sighupNotifier->setEnabled(false);
    char tmp;
    (void)read(this->sighupFileDescriptor[1], &tmp, sizeof(tmp));
...
void UnixSignalHandler::handleSigInt() {
    ...
    char tmp;
    (void)read(this->sigintFileDescriptor[1], &tmp, sizeof(tmp));
...
void UnixSignalHandler::handleSigTerm() {
    ...
    char tmp;
    (void)read(this->sigtermFileDescriptor[1], &tmp, sizeof(tmp));
